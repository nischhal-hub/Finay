            ...
            <div className='sidebar absolute left-0 w-52 h-auto bg-gradient-to-t from-sidebarBackground2 to-sidebarBackground'>
                <div className='page-urls font-urbanist'>
                    <ul className='mx-6 border-b-[1px] border-searchInputBorder'>
                        ...
                        <li key={i} className='text-textSecondary-200 cursor-pointer hover:text-accent transition py-2 first-of-type:pt-6 pb-2 last-of-type:pb-6'>
                            <NavLink className='flex' activeClassName="active" to={item.url}><span className='text-2xl'>{item.icon}</span><span className='text-base ml-4'>{item.label}</span></NavLink>
                            </li>))}
                    </ul>
                </div>
